...
import { fetchPurchaseOrders, updatePurchaseOrderStatus, deletePo, despatchPurchaseOrder, PaginatedApiResponse } from "../../services/api.service";
...
const [poToDispatch, setPoToDispatch] = useState<any | null>(null);
...
const handleStatusUpdate = async (poId: string, status: string) => {
  ...
  // Check if current PO has "PO Check" status
  const hasPoCheckStatus = currentPo?.statuses?.some((s: { status: string }) => s.status === "PO Check");
  if (hasPoCheckStatus) {
    toast.error("Cannot change status while PO is in 'PO Check'. Please edit the PO to fix amount mismatch first.");
    return;
  }

  // Prevent manual setting of "PO Check" status
  if (status === "PO Check") {
    toast.error("PO Check status can only be set by the system");
    return;
  }

  // Special handling for "Despatched/ Completed"
  if (status === "Despatched/ Completed") {
    setPoToDispatch(currentPo);
    setIsDispatchFormOpen(true);
    return;
  }

  // Normal status update for all other statuses
  const toastId = toast.loading(`Updating status...`);
  try {
    const response = await updatePurchaseOrderStatus(poId, status);
    const updatedStatuses = response.data?.statuses || response.statuses || [status];
    
    setPurchaseOrders(prevPOs => 
      prevPOs.map(p => {
        if (p.id === poId) {
          const newStatusArray = Array.isArray(updatedStatuses) 
            ? updatedStatuses.map(s => ({ status: s }))
            : [{ status: status }];
          const newCurrentStatus = Array.isArray(updatedStatuses) && updatedStatuses.length > 0 
...
const handleDispatchSubmit = async (deliveryDate: string, docketNumber: string) => {
  if (!poToDispatch) return;
  
  const toastId = toast.loading('Processing despatch...');
  try {
    // Call the backend API to save dispatch details
    const response = await despatchPurchaseOrder(poToDispatch.id, deliveryDate, docketNumber);
    
    // Update local state with the response from backend
    setPurchaseOrders(prevPOs => 
      prevPOs.map(p => {
        if (p.id === poToDispatch.id) {
          return { 
            ...p, 
            statuses: [{ status: 'Despatched/ Completed' }],
            current_status: 'Despatched/ Completed',
    ...
    toast.success(`PO ${poToDispatch.po_number} despatched successfully!`, { id: toastId });
    setIsDispatchFormOpen(false);
    setPoToDispatch(null);
  } catch (error: any) {
    console.error('❌ Dispatch error:', error);
    toast.error(error.message || 'Failed to process despatch', { id: toastId });
  }
};

const handleOpenDispatchForm = () => {
  setIsDispatchFormOpen(false);
  setPoToDispatch(null);
};
...
return (
  <>
    ...
    <EditPoForm open={isEditFormOpen} handleOpen={() => handleOpenEditForm(null)} po={poToEdit} onUpdate={handlePoUpdate} />
      ...
      handleOpen={handleOpenDispatchForm} 
